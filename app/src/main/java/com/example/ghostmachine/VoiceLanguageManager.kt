...
// Do NOT add new regex patterns/prefixes elsewhere - this is the one place.
    ...
    fun parseNormalizedCommand(command: String): Pair<String, String> {
        ...
        val videoCallRegex = Regex("^(?:video call|video chat) (.+)$")
        videoCallRegex.find(lower)?.let { match ->
            val contact = match.groupValues[1].trim()
            if (contact.isNotBlank()) return "video_call" to contact
        }

        // In an already open chat, these are intentionally one idempotent
        // action: type the requested text, then tap Send once.  "send him"
        // without a message stays blank and the service asks for the text.
        val sendRegex = Regex("^send(?: (?:him|her|it))? (.+)$")
        sendRegex.find(lower)?.let { match ->
            val message = match.groupValues[1].trim()
            if (message.isNotBlank()) return "type_and_send" to message
        }

        val typeAndSendRegex = Regex("^type (.+?) (?:and )?send(?: it)?$")
        typeAndSendRegex.find(lower)?.let { match ->
            val message = match.groupValues[1].trim()
            if (message.isNotBlank()) return "type_and_send" to message
        }
