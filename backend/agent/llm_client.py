...
def clean_model_json(model_text: str) -> str:
    ...
    # Remove reasoning blocks from some Ollama responses
    cleaned = re.sub(r'(?is)\bthinking\b.*?(\{|\[)', '{', cleaned, count=1)
    cleaned = re.sub(r'(?is)\banalysis\b.*?(\{|\[)', '{', cleaned, count=1)
    ...
    # Fix common malformed coordinate mistakes
    cleaned = re.sub(
        r'("x"\s*:\s*-?\d+(?:\.\d+)?),\s*-?\d+(?:\.\d+)?\s*,',
        r'\1,',
        cleaned
...
def call_vision_model(
    ...
    for attempt in range(3):
        nudge = "" if attempt == 0 else (
            ...
            "No markdown. No explanation. No reasoning text."
        )
        payload = make_payload(nudge)
        ...
        try:
            ...
            model_message = response_json.get("message", {})
            model_text = (model_message.get("content") or "").strip()
